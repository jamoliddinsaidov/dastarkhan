// error constants
export const SOMETHING_WENT_WRONG = 'Something went wrong. Please try again'
export const INTERNAL_SERVER_ERROR = 'Internal Server Error'
export const NOT_ALLOWED_BY_CORS = 'Not allowed by CORS'
export const ROUTE_DOESNT_EXIST = 'Route does not exist'

// connection constants
export const DB_CONNECTED = 'DB has been connected successfully'
export const DB_CONNECTION_FAILED = 'DB connection is failed, reason: \n'
export const SERVER_IS_RUNNING = 'Server is running on port'
export const SERVER_IS_CLOSING = 'Shutting down the server...'

// auth & user constants
export const EMAIL_ALREADY_EXISTS = 'Email already exists'
export const PASSWORD_DOES_NOT_MATCH = 'Provided password does not match'
export const USER_LOGGED_IN = 'User is successfully logged in'
export const AUTHORIZATION_HEADER_NOT_PROVIDED = 'Authorization header is not provided'
export const AUTHORIZATION_TOKEN_INVALID = 'Authorization token is invalid'
export const JWT_COOKIE_NOT_PROVIDED = 'JWT cookie is not provided'
export const JWT_COOKIE_IS_CLEARED = 'JWT cookie is cleared'
export const NO_USER_FOUND = 'No user found matching with the provided user ID or email'
export const USER_CREATED = 'User is successfully created'
export const USER_UPDATED = 'User is successfully updated'
export const NO_USERS_FOUND_MATCHING_EMAIL = 'No users found matching with the provided email'
export const INVALID_CREDENTIALS = 'Invalid credentials'
export const NO_EMAIL_PASSWORD = 'Please provide email and password'
export const LOGGED_OUT = 'User is logged out'
export const PROVIDE_EMAIL = 'Please provide an email'

// food review constants
export const REVIEW_ADDED = 'Food review is added successfully'
